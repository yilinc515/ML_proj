...
#simplified version of selnet (for 4x, 7500 epochs, 42 min, batch size=50)
class SupRes(nn.Module):
    ...
    def __init__(self, upscale_factor = 4):
        ...
        self.su1 = nn.Sequential(      
            ...
            nn.Conv2d(in_channels=128, out_channels=128,kernel_size=1, device='cuda'),                              
            nn.Sigmoid()                      
        )

        self.conv2=nn.Conv2d(in_channels=128, out_channels=80, kernel_size=3, stride=1, 
                             padding=1, device='cuda')
        
        self.su2 = nn.Sequential(      
            nn.PReLU(device='cuda'),
            nn.Conv2d(in_channels=80, out_channels=80, kernel_size=1, device='cuda'),                              
            nn.Sigmoid()                      
        )

        self.conv3=nn.Conv2d(in_channels=80, out_channels=64, kernel_size=3, stride=1, 
                             padding=1, device='cuda')
        
        self.su3 = nn.Sequential(      
            nn.PReLU(device='cuda'),
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1, device='cuda'),                              
            nn.Sigmoid()                      
        )

        self.conv4=nn.Conv2d(in_channels=64, out_channels=3*upscale_factor*upscale_factor, 
                             kernel_size=3, stride=1, padding=1, device='cuda')
